...
class AdminQualityController extends Controller
{
    ...
    public function reactivityIndex()
    {
        ...
        $current_question = $em
            ->getRepository(ReactivityQuestion::class)
            ->findOneBy([], ['id' => 'desc']);
        $questions = $this->processReactivityStats(
            $em
                ->getRepository(ReactivityQuestion::class)
                ->findAll([], ['date' => 'asc'])
        );
        ...
        if ($current_question != null) {
            $answers = $em
                ->getRepository(ReactivityAnswer::class)
                ->getReactivityAnswerListFromQuestion(
                    $current_question->getId()
                );
            if ($current_question->isFinished()) {
                if (
                    !$em
                        ->getRepository(ReactivityQuestion::class)
                        ->hasPoleStats($current_question->getId())
                ) {
                    $canGenerate = true;
                }
            } else {
        ...
        return $this->render('Quality/Default/reactivity_index.html.twig', [
            ...
            'displayQuestionForm' => !$questionOngoing,
    ...
    public function moodIndex(MoodReasons $mood_service)
    {
        $em = $this->getDoctrine()->getManager();
        $current_question = $em
            ->getRepository(MoodQuestion::class)
            ->findOneBy([], ['id' => 'desc']);
        $questions = $this->processMoodStats(
            $em
                ->getRepository(MoodQuestion::class)
                ->findAll([], ['date' => 'asc'])
        );
        ...
        if ($current_question != null) {
            $answers = $em
                ->getRepository(MoodAnswer::class)
                ->getMoodAnswerListFromQuestion($current_question->getId());
            if ($current_question->isFinished()) {
    ...
    {
        $success_message =
            'Votre question pour le test de réactivité a été envoyée';
        $em = $this->getDoctrine()->getManager();
        $reactivity_question = new ReactivityQuestion();

        $form = $this->createForm(
            ReactivityQuestionType::class,
            $reactivity_question
        );
        ...
        if ('POST' == $request->getMethod()) {
            ...
            if ($form->isValid()) {
                ...
                $message = (new \Swift_Message(
                    '[Test de Réactivité] Nouveau test'
                ))
    ...
    public function newMoodQuestion()
    {
        ...
        $this->addFlash('success', $success_message);
        return $this->redirectToRoute('quality_mood_admin', []);
    }
    ...
    {
        $success_message =
            'Les statistiques de réactivité de ce mois ont bien été générées';
        $warning_message =
            'Les statistiques de réactivité de ce mois ont déjà été générées';

        $em = $this->getDoctrine()->getManager();
        $question = $em->getRepository(ReactivityQuestion::class)->find($id);
        $question_expiration = $question
            ->getAskedAt()
        ...
        if (!$question->getHasPolestats()) {
            ...
            foreach ($poles as $pole) {
                $answers = $em
                    ->getRepository(ReactivityAnswer::class)
                    ->getReactivityAnswerListFromQuestionPole(
                        $question->getId(),
                        $pole->getId()
                    );
                $members_nb = count(
                    $em
                        ->getRepository(Membre::class)
                        ->getMembresPoleCourant($pole->getId())
                );
                $answers_count = count($answers);
                $no_answer =
                    (1.0 * ($members_nb - $answers_count)) / $members_nb;
                $bad_answer = 0;
                $good_answer = 0;
    ...
    {
        $success_message =
            'Les statistiques de moral de ce mois ont bien été générées';
        $warning_message =
            'Les statistiques de moral de ce mois ont déjà été générées';
        ...
        if (!$question->getHasMoodStats()) {
            ...
            foreach ($poles as $pole) {
                $answers = $em
                    ->getRepository(MoodAnswer::class)
                    ->getMoodAnswerListFromQuestionPole(
                        $question->getId(),
                        $pole->getId()
                    );
                $members_nb = count(
    ...
    public function createPoleStat(
        $current_prom,
        $month,
        $question,
        $pole,
    ...
    public function createMoodStat(
        $current_prom,
        $month,
        $question,
        $pole,
